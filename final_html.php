...
<?php
global $post;
$header_check = get_post_meta( $post->ID, '_ng_header', true );
$l_sidebar_check = get_post_meta( $post->ID, '_ng_l_sidebar', true );
$upper_html_check = get_post_meta( $post->ID, '_ng_upper_html', true );

$lower_html_check = get_post_meta( $post->ID, '_ng_lower_html', true );
$r_sidebar_check = get_post_meta( $post->ID, '_ng_r_sidebar', true );
$footer_check = get_post_meta( $post->ID, '_ng_footer', true );

// Create a new filtering function that will add our where clause to the query
function formatted_date($where = '') {
	global $post;
	$start_date = get_post_meta( $post->ID, '_ng_start_date', true );
	$end_date = get_post_meta( $post->ID, '_ng_end_date', true );
	$start_date_format = date("Y-m-d", strtotime($start_date));
	$end_date_format = date("Y-m-d", strtotime($end_date));
	$where .= " AND post_date >= '" . $start_date_format ."' AND post_date < '".$end_date_format."'";
	return $where;
}

add_filter( 'posts_where', 'formatted_date' );
...
remove_filter( 'posts_where', 'formatted_date' );
?>
...
<tbody>
	<tr>
		<td align="center">
			...
			$above_posts = get_post_meta( $post->ID, '_ng_above_posts', true );
			$post_layout = get_option('ng_layout_html');
			$below_posts = get_post_meta( $post->ID, '_ng_below_posts', true );
			...
			<table border="0" style="background:<?php if($mbg == '0') {echo $bgc;} else {?>url('<?php echo $bgimg_url;?>')<?php }?>; font-family:<?php echo get_option('ng_font_family');?>; font-size:<?php echo get_option('ng_font_size')?>px; color:<?php echo get_option('ng_text_color');?>" width="" cellpadding="<?php echo get_option('ng_pbmb')?>" cellspacing="0" >
				...
				<tr>
					...
					<td valign="top" <?php if (!empty($content_width)) {?> width="<?php echo $content_width?>px"<?php }?>>
						<table border="0" width="100%" cellpadding="0" cellspacing="0" style="font-size:<?php echo get_option('ng_font_size')?>px;">
							<tbody>
								<?php if (($upper_html_check == '1') && (!empty($body_up))) {?>
								<tr>
									<td><?php echo $body_up;?></td>
								</tr>
								<?php } ?>
								<?php if (($upper_html_check == '1') && (!empty($above_posts))){?>
								<tr>
									<td><?php echo $above_posts;?></td>
								</tr>
								<tr>
								<?php }?>	
								<td><?php								
								...
								<?php if (($lower_html_check == '1') && (!empty($below_posts))){?>
								<tr>
									<td><?php echo $below_posts;?></td>
								</tr>
								<?php } ?>
								<?php if (($lower_html_check == '1') && (!empty($body_low))){?>
								<tr>
									<td><?php echo $body_low;?></td>
								</tr>
								<?php } ?>
							</tbody>							
						</table>
